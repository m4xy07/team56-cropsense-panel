...
import { AQIChart } from './Chart3';
import { useEffect, useState } from 'react';

  
  export function MorphingDialogBasicThree() {

    const [aqi, setAQI] = useState<number | null>(null);
        
          useEffect(() => {
            const fetchAQI = async () => {
              console.log("Fetching AQI..."); // Debug log
              try {
                ...
                if (Array.isArray(data) && data.length > 0) {
                  const latestAQI = data[data.length - 1]?.aqi; // FIXED HERE

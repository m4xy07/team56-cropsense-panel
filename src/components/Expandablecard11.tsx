...
import { NitrogenChart } from './Charts11';

export function MorphingDialogBasicEleven() {

   const [npk_uptake_nitrogen, setNitrogen] = useState<number | null>(null);
              
                useEffect(() => {
                  const fetchNitrogen = async () => {
                    try {
                      ...
                      if (Array.isArray(data) && data.length > 0) {
                        const latestNitrogen = data[data.length - 1]?.npk_uptake_nitrogen; // FIXED HERE
                        setNitrogen(latestNitrogen);
                      } else {
                        console.warn("API returned an empty or invalid response.");
                      }
                    } catch (error) {
                      console.error("Error fetching nitrogen:", error);
                    }
                  };
        ...
        <div className='flex grow flex-row items-center justify-between px-3 py-2'>
          <div>
            ...
            <MorphingDialogSubtitle className='text-zinc-700 text-center dark:text-zinc-400'>
               {npk_uptake_nitrogen !== null ? `${npk_uptake_nitrogen}` : 'Loading...'}
            </MorphingDialogSubtitle>
          </div>

...
import { PotassiumChart } from './Charts13';


export function MorphingDialogBasicThirteen() {

   const [npk_uptake_potassium, setpotassium] = useState<number | null>(null);
              
                useEffect(() => {
                  const fetchpotassium = async () => {
                    try {
                      ...
                      if (Array.isArray(data) && data.length > 0) {
                        const latestpotassium = data[data.length - 1]?.npk_uptake_potassium; // FIXED HERE
                        setpotassium(latestpotassium);
                      } else {
                        console.warn("API returned an empty or invalid response.");
                      }
                    } catch (error) {
                      console.error("Error fetching potassium:", error);
                    }
                  };
        ...
        <div className='flex grow flex-row items-center justify-between px-3 py-2'>
          <div>
            ...
            <MorphingDialogSubtitle className='text-zinc-700 text-center dark:text-zinc-400'>
               {npk_uptake_potassium !== null ? `${npk_uptake_potassium}` : 'Loading...'}
            </MorphingDialogSubtitle>
          </div>

  ...
  React.useEffect(() => {
    async function fetchData() {
      try {
        ...
        const formattedData = data.map((entry: { time: string; aqi: number }) => ({
    ...
    <Card>
      <CardHeader className="flex flex-col items-stretch space-y-0 border-b p-0 sm:flex-row">
        <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-5 sm:py-6">
          <CardTitle>Air Quality Index (AQI) Over Time</CardTitle>
          <CardDescription>Real-time AQI readings</CardDescription>
        </div>
      </CardHeader>
